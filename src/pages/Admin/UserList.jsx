...
export default function UserList() {
  ...
  const fetchUsers = useCallback(async () => {
    setLoading(true);
    try {
      const res = await axios.get(`${import.meta.env.VITE_API_URL}/admin/users-total-last-35-days`, {
  ...
  const handleDelete = async (targetUser) => {
    ...
    try {
      toast.promise(
        axios.delete(`${import.meta.env.VITE_API_URL}/admin/user/${targetUser.userId}`, {
          headers: { Authorization: `Bearer ${user.token}` },
        }),
        {
